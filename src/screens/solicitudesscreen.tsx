...
const SolicitudesScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const cargarSolicitudes = useCallback(async () => {
    try {
      ...
      if (error) {
        console.error('Error cargando solicitudes:', JSON.stringify(error));
  ...
  const onRefresh = () => { setRefreshing(true); cargarSolicitudes(); };

  const cambiarEstado = async (id: string, nuevoEstado: 'CONFIRMADA' | 'CANCELADA') => {
      ...
      [
        ...
        },
        {
          ...
          onPress: () => {
            ...
            const vehNombre = s.marca && s.modelo
              ? `${s.marca} ${s.modelo}`
              : 'el vehículo';
  ...
  const getEstadoStyle = (estado: string) => {
    switch (estado) {
      case 'CONFIRMADA': return { bg: '#14532d', text: '#4ade80' };
      case 'CANCELADA':  return { bg: '#450a0a', text: '#f87171' };
      default:           return { bg: '#451a03', text: '#fbbf24' };
    }
  };

  const getNombreVehiculo = (s: Solicitud) => {
    if (s.marca && s.modelo) return `${s.marca} ${s.modelo}`;
    if (s.marca)             return s.marca;
    return 'Vehículo';
  };
          ...
          <View style={styles.cardHeaderLeft}>
            ...
            <View>
              ...
              <Text style={styles.cardFecha}>
                {formatFecha(s.fecha_visita)} · {s.hora_visita?.slice(0, 5) || '—'}
              </Text>
            </View>
          </View>
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      <Header title="Solicitudes de visita" onBackPress={() => navigation.goBack()} />
      ...
      <View style={styles.filtros}>
        {ESTADOS.map(e => (
          <TouchableOpacity
            key={e.key}
            style={[styles.filtroBtn, filtro === e.key && { backgroundColor: e.color, borderColor: e.color }]}
            onPress={() => setFiltro(e.key)}
          >
            <Text style={[styles.filtroText, filtro === e.key && { color: '#fff' }]}>
              {e.label}
            </Text>
          </TouchableOpacity>
        ))}
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySub}>
              {filtro === 'TODOS' ? 'No hay solicitudes de visita' : `No hay solicitudes ${filtro.toLowerCase()}s`}
            </Text>
          </View>
        }
      />
...
const styles = StyleSheet.create({
  ...
  loadingText:   { marginTop: spacing.lg, color: Colors.textSecondary, fontSize: 14 } as TextStyle,
  filtros:       { flexDirection: 'row', paddingHorizontal: spacing.lg, paddingVertical: spacing.md, gap: spacing.sm, flexWrap: 'wrap' } as ViewStyle,
  filtroBtn:     { paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, borderRadius: 20, borderWidth: 1, borderColor: Colors.border } as ViewStyle,
  filtroText:    { fontSize: 13, fontWeight: '600', color: Colors.textSecondary } as TextStyle,
  contador:      { fontSize: 12, color: Colors.textTertiary, paddingHorizontal: spacing.lg, marginBottom: spacing.sm } as TextStyle,
  list:          { paddingHorizontal: spacing.lg, paddingBottom: spacing.xxl } as ViewStyle,
  card:          { backgroundColor: Colors.secondaryBackground, borderRadius: 16, padding: spacing.lg, marginBottom: spacing.lg } as ViewStyle,
  cardHeader:    { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: spacing.lg } as ViewStyle,
  cardHeaderLeft:{ flexDirection: 'row', alignItems: 'center', gap: spacing.md, flex: 1 } as ViewStyle,
  cardIcon:      { width: 40, height: 40, borderRadius: 20, backgroundColor: Colors.tertiaryBackground, justifyContent: 'center', alignItems: 'center' } as ViewStyle,
  cardVehiculo:  { fontSize: 15, fontWeight: '700', color: Colors.textPrimary } as TextStyle,
  cardFecha:     { fontSize: 12, color: Colors.textSecondary, marginTop: 2 } as TextStyle,
  estadoBadge:   { paddingHorizontal: spacing.md, paddingVertical: 4, borderRadius: 6 } as ViewStyle,
  estadoText:    { fontSize: 10, fontWeight: '800', letterSpacing: 0.5 } as TextStyle,
  clienteInfo:   { backgroundColor: Colors.tertiaryBackground, borderRadius: 10, padding: spacing.md, marginBottom: spacing.lg, gap: spacing.sm } as ViewStyle,
  infoRow:       { flexDirection: 'row', alignItems: 'center', gap: spacing.sm } as ViewStyle,
  infoText:      { fontSize: 13, color: Colors.textSecondary } as TextStyle,
  acciones:      { flexDirection: 'row', gap: spacing.sm } as ViewStyle,
  accionBtn:     { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, paddingVertical: spacing.md, borderRadius: 10 } as ViewStyle,
  ...
  empty:         { alignItems: 'center', paddingVertical: spacing.xxl * 2 } as ViewStyle,
  emptyTitle:    { fontSize: 18, fontWeight: '700', color: Colors.textPrimary, marginTop: spacing.lg } as TextStyle,
  emptySub:      { fontSize: 14, color: Colors.textSecondary, marginTop: spacing.sm, textAlign: 'center' } as TextStyle,
});
